...
class TaskManager:
    ...
    def set_result(self, task_id: str, result: Any, status: TaskStatus):
        """
        设置任务结果
        """
        ...
        # 保存结果并更新状态
        self.task_repo.set_result(task_id, str(result), status)
        # 任务完成后清理worker
        with self.worker_lock:
    ...
    def change_priority(self, task_id: str, priority: Priority, preemptive: bool = False):
        ...
        # 更新数据库中的优先级
        self.task_repo.update_priority(task_id, priority)
    ...
    def _perform_preemption(self):
        ...
        with self.worker_lock:
            ...
            for worker in self.workers.values():
                ...
                if task_priority.value > lowest_priority:  # priority值越大，优先级越低
                    lowest_priority = task_priority.value
                    lowest_priority_worker = worker

            # 比较待执行任务和当前最低优先级任务的优先级
            if lowest_priority_worker and highest_priority_task.priority.value < lowest_priority:
                # 待执行任务优先级更高，执行抢占
    ...
    def _restore_tasks_from_db(self):
        """从数据库恢复未完成的任务到队列"""
        pending_tasks = self.task_repo.restore_pending_tasks()
        paused_tasks = self.task_repo.restore_paused_tasks()
        with self.queue_lock:
            for task in pending_tasks:
                heapq.heappush(self.pending_queue, task)
            for task in paused_tasks:
                heapq.heappush(self.paused_queue, task)
        if pending_tasks or paused_tasks:
            logger.info(f"[TaskManager] 从数据库恢复 {len(pending_tasks)} 个待执行任务，{len(paused_tasks)} 个暂停任务")
