...
async def create_file(path: str, content: str):
    ...
    async with get_vfs_lock():
        async with db_pool.get_conn() as conn:
            ...
            await DiffTable.operate(record, _conn=conn)
...
async def delete_file(path: str):
    ...
    async with get_vfs_lock():
        async with db_pool.get_conn() as conn:
            ...
            # 在暂存区中标记删除
            await staging_area.delete(path, _conn=conn)
        ...
        async with db_pool.get_conn() as conn:
            ...
            if source_staging_path is None:
                ...
                if not os.path.exists(source_path):
                    return f"ERROR：原文件{source_path}不存在"
                # 从暂存区获取一条路径
                await staging_area.register(source_path, _conn=conn)

            # 修改对应的路径映射关系
            await staging_area.rename(source_path, target_path, _conn=conn)
            copy_mapping = get_copy_mapping()
            await copy_mapping.rename(source_path, target_path, _conn=conn)
...
async def copy_file(source_path: str, target_path: str):
    ...
    async with get_vfs_lock():
        async with db_pool.get_conn() as conn:
            ...
            await staging_area.register(target_path, _conn=conn)

            # 写入一条复制记录，用于写时复制
            copy_mapping = get_copy_mapping()
            await copy_mapping.register(source_path, target_path, _conn=conn)
...
async def move_file(source_path: str, target_path: str):
    ...
    async with get_vfs_lock():
        async with db_pool.get_conn() as conn:
            ...
            # 在暂存区中标记删除
            await staging_area.delete(source_path, _conn=conn)
            ...
            await DiffTable.operate_batch([record1, record2], _conn=conn)
...
async def mkdir(path: str):
    ...
    async with get_vfs_lock():
        async with db_pool.get_conn() as conn:
            ...
            # 在暂存区为path占位
            await staging_area.register_dir(path, _conn=conn)
...
async def delete_dir(path: str):
    ...
    async with get_vfs_lock():
        async with db_pool.get_conn() as conn:
            ...
            # 在暂存区中删除
            await staging_area.delete_dir(path, _conn=conn)
        ...
        async with db_pool.get_conn() as conn:
            ...
            if staging_path is None:
                ...
                if not os.path.exists(source_path):
                    return f"ERROR：目录{source_path}不存在"
                # 从暂存区获取一条路径，并拷贝原目录
                staging_path = await staging_area.register_dir(source_path, _conn=conn)

            # 修改对应的映射关系
            await staging_area.rename_dir(source_path, target_path, _conn=conn)
            await get_copy_mapping().rename_dir(source_path, target_path, _conn=conn)
    ...
    async with get_vfs_lock():
        async with db_pool.get_conn() as conn:
            ...
            if target_staging_path:
                ...
                return f"ERROR：命名冲突，目标目录{target_path}已存在"
            # 在暂存区为target_path注册
            await staging_area.register_dir(target_path, _conn=conn)

            # 写入一条复制记录，用于写时复制
            copy_mapping = get_copy_mapping()
            await copy_mapping.register_dir(source_path, target_path, _conn=conn)
            ...
            for root, dirs, files in os.walk(source_path):
                ...
                for dir_name in dirs:
                    ...
                    # 在暂存区为target_dir占位
                    await staging_area.register_dir(target_dir, _conn=conn)
...
async def move_dir(source_path: str, target_path: str):
    ...
    async with get_vfs_lock():
        async with db_pool.get_conn() as conn:
            ...
            # 在暂存区中删除原目录
            await staging_area.delete_dir(source_path, _conn=conn)
