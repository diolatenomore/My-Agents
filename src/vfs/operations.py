...
def create_file(path: str, content: str):
    ...
    # 在暂存区为path分配一条路径
    staging_path = StagingArea.register(path)
...
def delete_dir(path: str):
    ...
    if CopyMapping.need_copied_dir(path):
        CopyMapping.mark_copied_dir(path)

    # 在暂存区中删除
    StagingArea.delete_dir(path)
