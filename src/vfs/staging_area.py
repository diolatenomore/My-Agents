...
class StagingArea:
    ...
    def delete_dir(path: str):
        ...
        for dir_path in list(StagingArea.deleted_dir_mapping.keys()):
            if dir_path.startswith(path + "/"):
                del StagingArea.mapping[dir_path]
    ...
    def rename(old_path: str, new_path: str) -> None:
        ...
        # 标记原路径为已删除
        StagingArea.deleted_mapping[old_path] = True
        ...
        try:
            ...
            if existing:
                ...
                VALUES (?, ?, ?, ?)
                ''', (StagingArea.task_id, old_path, False, True))

            # 插入新路径记录
            cursor.execute('''
            INSERT INTO staging_records (task_id, path, staging_path, is_dir, deleted)
            VALUES (?, ?, ?, ?, ?)
            ''', (StagingArea.task_id, new_path, StagingArea.mapping[new_path], False, False))
    ...
    def rename_dir(old_dir_path: str, new_dir_path: str) -> None:
        ...
        # 标记原目录被删除
        StagingArea.deleted_dir_mapping[old_dir_path] = True
        ...
        for old_path in list(StagingArea.mapping.keys()):
            if old_path.startswith(old_dir_path + "/"):
                ...
                if old_path in StagingArea.deleted_mapping:
                    ...
                    StagingArea.deleted_mapping[old_path] = True
                elif old_path in StagingArea.deleted_dir_mapping:
                    StagingArea.deleted_dir_mapping[new_sub_path] = StagingArea.deleted_dir_mapping[old_path]
                    StagingArea.deleted_dir_mapping[old_path] = True
        ...
        try:
            ...
            if existing:
                ...
                INSERT INTO staging_records (task_id, path, staging_path, is_dir, deleted)
                VALUES (?, ?, ?, ?)
                ''', (StagingArea.task_id, old_dir_path, True, True))

            # 插入新目录记录
            cursor.execute('''
            INSERT INTO staging_records (task_id, path, staging_path, is_dir, deleted)
            VALUES (?, ?, ?, ?, ?)
            ''', (StagingArea.task_id, new_dir_path, StagingArea.mapping[new_dir_path], True, False))
            ...
            for row in cursor.fetchall():
                ...
                INSERT INTO staging_records (task_id, path, staging_path, is_dir, deleted)
                VALUES (?, ?, ?, ?, ?)
                ''', (StagingArea.task_id, new_sub_path, row['staging_path'], row['is_dir'], False))
