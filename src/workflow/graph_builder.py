...
class GraphBuilder:
    ...
    @staticmethod
    def create_graph(task: Task) -> Tuple[StateGraph, Dict[str, Any], Optional[Callable]]:
        mapping = {
        ...
        if task.task_type in mapping:
            graph, state, func = mapping[task.task_type](task)
            return graph, state, func if func is not None else default_cleanup_func
        else:
