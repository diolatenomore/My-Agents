...
async def delete_file(path: str, _task_id: Optional[str]= None) -> str:
    """删除指定文件"""
    return ops.delete_file(path, _task_id)

@tool(args_schema=RenameFileInput)
async def rename_file(old_path: str, new_path: str, _task_id: Optional[str]= None) -> str:
    """重命名文件，只支持在同一目录下重命名文件，不支持跨目录重命名"""
    return ops.rename_file(old_path, new_path, _task_id)

@tool(args_schema=ModifyFileInput)
async def modify_file(path: str, content: str, _task_id: Optional[str]= None) -> str:
    """修改文件内容"""
    return ops.modify_file(path, content, _task_id)

@tool(args_schema=CopyFileInput)
async def copy_file(source_path: str, target_path: str, _task_id: Optional[str]= None) -> str:
    """复制文件到指定目标路径"""
    return ops.copy_file(source_path, target_path, _task_id)

@tool(args_schema=MoveFileInput)
async def move_file(source_path: str, target_path: str, _task_id: Optional[str]= None) -> str:
    """移动文件到指定目标路径"""
    return ops.move_file(source_path, target_path, _task_id)

@tool(args_schema=MkdirInput)
async def mkdir(path: str, _task_id: Optional[str]= None) -> str:
    """创建目录"""
    print(f"task_id:{_task_id}")
    return ops.mkdir(path, _task_id)

@tool(args_schema=DeleteDirInput)
async def delete_dir(path: str, _task_id: Optional[str]= None) -> str:
    """删除目录"""
    return ops.delete_dir(path, _task_id)

@tool(args_schema=RenameDirInput)
async def rename_dir(old_path: str, new_path: str, _task_id: Optional[str]= None) -> str:
    """重命名目录，只支持在同一目录下重命名目录，不支持跨目录重命名"""
    return ops.rename_dir(old_path, new_path, _task_id)

@tool(args_schema=CopyDirInput)
async def copy_dir(source_path: str, target_path: str, _task_id: Optional[str]= None) -> str:
    """复制目录到指定目标路径"""
    return ops.copy_dir(source_path, target_path, _task_id)

@tool(args_schema=MoveDirInput)
async def move_dir(source_path: str, target_path: str, _task_id: Optional[str]= None) -> str:
    """移动目录到指定目标路径"""
    return ops.move_dir(source_path, target_path, _task_id)
